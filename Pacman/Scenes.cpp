...
namespace dae
{
	...
	void HighScoreList::LoadScene()
	{
		...
		auto logoTrans = logoObj->AddComponent<dae::TransformComponent>();
		logoTrans->SetLocalPosition(glm::vec2{ 0.f, 0.f});
		logoObj->SetParent(sceneRoot.get());

		auto font = dae::ResourceManager::GetInstance().LoadFont("ArcadeFont.ttf", 16);

		auto highScores{ HighScoreIO::LoadHighScoreTable("Data/highscores.hs") };
		auto rootObj = std::make_shared<GameObject>();
		auto rootTrans = rootObj->AddComponent<TransformComponent>();
		...
		auto button1Obj = std::make_shared<dae::GameObject>();
		auto button1Trans = button1Obj->AddComponent<dae::TransformComponent>();
		button1Trans->SetLocalPosition(glm::vec2{650.f, 600.f});
		auto button1 = button1Obj->AddComponent<dae::ButtonComponent>();
		button1->SetButtonExtend(glm::vec2{10.f, 10.f});
		button1->SetButtonFont(font);
		button1->SetButtonText("Back");
		button1->SetNormalColor(255, 255, 255, 255);
		button1->SetHighlightColor(255, 255, 0, 255);
		button1->SetOnClick([]()
			{
		...
		{
			//Ranking
			auto rankObj = std::make_shared<GameObject>();

			auto rankTrans = rankObj->AddComponent<dae::TransformComponent>();
			rankTrans->SetLocalPosition(glm::vec2{ 0.f, 30.f * idx});

			auto rankText = rankObj->AddComponent<dae::TextComponent>();
			rankText->SetFont(font);
			rankText->SetColor(255, 255, 255, 255);

			std::stringstream rank{};
			rank << (idx + 1);
			rankText->SetText(rank.str());

			rankObj->AddComponent<dae::TextureRenderComponent>();
			rankObj->SetParent(rootObj.get(), false);

			//Level
			auto levelObj = std::make_shared<GameObject>();

			auto levelTrans = levelObj->AddComponent<dae::TransformComponent>();
			levelTrans->SetLocalPosition(glm::vec2{ 100.f, 30.f * idx });


			auto levelText = levelObj->AddComponent<dae::TextComponent>();
			levelText->SetFont(font);
			levelText->SetColor(255, 255, 255, 255);

			std::stringstream level{};
			level << highScores[idx].level;
			levelText->SetText(level.str());

			levelObj->AddComponent<dae::TextureRenderComponent>();
			levelObj->SetParent(rootObj.get(), false);

			//Score
			auto scoreObj = std::make_shared<GameObject>();

			auto scoreTrans = scoreObj->AddComponent<dae::TransformComponent>();
			scoreTrans->SetLocalPosition(glm::vec2{ 200.f, 30.f * idx });
			...
			score << highScores[idx].score;
			scoreText->SetText(score.str());

			scoreObj->AddComponent<dae::TextureRenderComponent>();
			scoreObj->SetParent(rootObj.get(), false);

			////Name
			auto nameObj = std::make_shared<GameObject>();

			auto nameTrans = nameObj->AddComponent<dae::TransformComponent>();
			nameTrans->SetLocalPosition(glm::vec2{ 300.f, 30.f * idx });

			auto nameText = nameObj->AddComponent<dae::TextComponent>();
			nameText->SetFont(font);
			nameText->SetColor(255, 255, 255, 255);

			std::stringstream name{};
			name << highScores[idx].name;
			nameText->SetText(name.str());

			nameObj->AddComponent<dae::TextureRenderComponent>();
			nameObj->SetParent(rootObj.get(), false);
		}
		...
		auto rankTrans = rankObj->AddComponent<dae::TransformComponent>();
		rankTrans->SetLocalPosition(glm::vec2{ 0.f, -30.f});
		...
		auto levelTrans = levelObj->AddComponent<dae::TransformComponent>();
		levelTrans->SetLocalPosition(glm::vec2{ 100.f, -30.f });
		...
		auto scoreTrans = scoreObj->AddComponent<dae::TransformComponent>();
		scoreTrans->SetLocalPosition(glm::vec2{ 200.f, -30.f});
		...
		auto nameTrans = nameObj->AddComponent<dae::TransformComponent>();
		nameTrans->SetLocalPosition(glm::vec2{ 300.f, -30.f});
